...
class ednaClass :
    ...
    def run(self,
        ...
        initializeGPIO(self.ledGPIO,self.flowSensor,
        ...
        self.eventLog.info("[%s] - Flipping valve open. Valve GPIO: %d" % (str(currentTimeString()),valve))
        ...
        self.eventLog.info("[%s] - Turning water pump on. Water pump GPIO: %d" % (str(currentTimeString()),self.sampleMotor))
        
        #WATER PUMPING 
        (totalWaterL,elapsedTime,pressure) = recordFlow(self.flowSensor,self.maxDiffPress,self.tWLiters,
        ...
        getSampleDone(self.ledGPIO, sampleNum) #LED
            
        elapsedTime = elapsedTime
        if (totalWaterL < self.tWLiters):
            sampleMotorP.stop()
            self.eventLog.info("[%s] - No water flow detected,flipping valve/motor off" % (str(currentTimeString())))
            ...
            self.eventLog.info("[%s] - ERROR: Solution lost. Faking successfull water intake to intake ethanol. totalWaterL: %f," % (str(currentTimeString()),totalWaterL))
            successfull = False
            
        if (pressure == False):
            self.eventLog.info("[%s] - Pressure too high" % (str(currentTimeString())))
            print ("[%.3f] - ERROR: Solution lost. Water pressure too high" % elapsedTime)
            totalWaterL = self.tWLiters
            self.eventLog.info("[%s] - ERROR: Solution lost. Water pressure too high" % (str(currentTimeString())))
            successfull = False
         
        if (totalWaterL >= self.tWLiters):            
            print ("[%.3f] - Prepping for ethanol" % elapsedTime)
            self.eventLog.info("[%s] - Prepping for ethanol" % str(currentTimeString()))
            ...
            closeValve(valveEn,valve,valveGnd)
            print ("[%.3f] - Flipping valve closed. Valve GPIO: %d" % (elapsedTime, valve))
            self.eventLog.info("[%s] - Flipping valve closed. Valve GPIO: %d" % (str(currentTimeString()),valve))
            ...
            self.eventLog.info("[%s] - Turning water pump off. Water pump GPIO: %d" % (str(currentTimeString()),self.sampleMotor))
            GPIO.setmode(GPIO.BCM)  #pin number
            ...
            openValve(valveEthaEn,valveEtha,valveEthaGnd)
            print ("[%.3f] - Flipping valve open. Valve GPIO: %d" % (elapsedTime,valveEtha))
            self.eventLog.info("[%s] - Flipping valve open. Valve GPIO: %d" % (str(currentTimeString()),valveEtha))
            ...
            self.eventLog.info("[%s] - Turning solution pump on. Solution pump GPIO: %d" % (str(currentTimeString()), self.ethanolMotor))

            #ETHANOL PUMPING
            (totalSoluL,elapsedTime,pressure) = recordFlow(self.flowSensor,self.maxDiffPress,self.tSLiters,
                                                           self.eventLog,self.dataFile,elapsedTime,
            ...
            getEthanolDone(self.ledGPIO, sampleNum) #LED
            
            if (totalSoluL < self.tSLiters):
                ethanolMotorP.stop()
                print ("[%.3f] - ERROR: No ethanol flow detected" % (elapsedTime))
                self.eventLog.info("[%s] - ERROR: No ethanol flow detected" % (str(currentTimeString())))
                totalSoluL = self.tSLiters
                self.eventLog.info("[%s] - Faking successfull solution intake to close all valves/motors. totalWaterL:%f," % (str(currentTimeString()),totalSoluL))
                successfull = False
            ...
            if (totalSoluL >= self.tSLiters):
                ...
                self.eventLog.info("[%s] - Prepping for system shutdown" % (str(currentTimeString())))
                ...
                closeValve(valveEthaEn,valveEtha,valveEthaGnd)
                print ("[%.3f] - Flipping valve closed. Valve GPIO: %d" % (elapsedTime,valveEtha))
                self.eventLog.info("[%s] - Flipping valve closed. Valve GPIO: %d" % (str(currentTimeString()),valveEtha))
                ...
                self.eventLog.info("[%s] - Turning solution pump off, Solution pump GPIO: %d" % (str(currentTimeString()), self.ethanolMotor))
